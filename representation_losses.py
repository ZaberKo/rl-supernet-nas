...
class LatentDynamicsPredictor(nn.Module):
    ...
    def forward(
        ...
        if action_features.dim() != 3:
            raise ValueError(
                "action_features must have shape [batch, action_dim] or [batch, horizon, action_dim]."
            )
        ...
        for step_index in range(action_features.size(1)):
            latent = self.net(
                torch.cat([latent, action_features[:, step_index]], dim=-1)
            )
...
def latent_dynamics_loss(
    ...
    if distance.dim() != 2:
        raise ValueError(
            "latent dynamics distance must have shape [batch] or [batch, horizon]."
        )

    weights = torch.ones(distance.size(1), dtype=distance.dtype, device=distance.device)
    if beta_weights is not None:
        weights = torch.as_tensor(
            beta_weights, dtype=distance.dtype, device=distance.device
        )
    ...
    if sample_weights is not None:
        ...
        if weights.shape != distance.shape:
            raise ValueError(
                "sample_weights shape must match the latent dynamics distance shape."
            )
...
def cosine_kd_loss(
    ...
    if sample_weights is None:
        return distance.mean()
    weights = sample_weights.to(dtype=distance.dtype, device=distance.device).view_as(
        distance
    )
    return (distance * weights).sum() / weights.sum().clamp_min(1.0)
...
def encode_action_batch(actions: torch.Tensor, action_space: gym.Space) -> torch.Tensor:
    if isinstance(action_space, gym.spaces.Discrete):
        return F.one_hot(
            actions.long().view(-1), num_classes=int(action_space.n)
        ).float()
    if isinstance(action_space, gym.spaces.Box):
    ...
    if isinstance(action_space, gym.spaces.MultiDiscrete):
        ...
        for index, classes in enumerate(action_space.nvec):
            parts.append(
                F.one_hot(actions[:, index].long(), num_classes=int(classes)).float()
            )
